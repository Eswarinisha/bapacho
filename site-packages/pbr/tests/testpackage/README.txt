...
selenium.Library* ``__version__`` (the release version)
selenium.Library* ``__svn_revision__`` (the SVN revision info as returned by the ``svnversion``
  command)
selenium.Library* ``__svn_full_info__`` (as returned by the ``svn info`` command)
selenium.Library* ``__setup_datetime__`` (the date and time that setup.py was last run).
